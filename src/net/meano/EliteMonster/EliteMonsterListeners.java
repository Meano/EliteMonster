...
public class EliteMonsterListeners implements Listener{
	...
	public void onEntityDamage(EntityDamageEvent event){
		...
		if(DamageEntity instanceof Monster){
			//Bukkit.broadcastMessage("Cause: "+ event.getCause() + "Type: " + event.getEntityType().toString() + "Damage: " + event.getDamage());
	...
	public void onEntityDeath(EntityDeathEvent event){
		...
		boolean isUnNormalDead = (DeadEntity.getKiller()==null)&&(DeadEntity instanceof Monster)&&(!(DeadEntity instanceof Zombie))&&(!(DeadEntity instanceof Skeleton));
		if((DeadEntity.hasMetadata("isFall")||isUnNormalDead)){
		...
		if(DeadEntity.getKiller()!=null&&DeadEntity.getKiller() instanceof Player)			
			if(DeadEntity.isCustomNameVisible()){
				...
				if(DeadEntity.getKiller().getItemInHand().getItemMeta().hasDisplayName()){
					KillerAmor = new StringBuffer()
								.append(ChatColor.RED)
								.append(" 用传说中的")
								.append(ChatColor.AQUA)
								.append(" [")
								.append(DeadEntity.getKiller().getItemInHand().getItemMeta().getDisplayName())
								.append("]").toString();
				}
				int Exp = 15+(int)(Math.random()*((Damageable)DeadEntity).getMaxHealth())/2;
				Bukkit.broadcastMessage(new StringBuffer()
					.append(DeadEntity.getCustomName())
					.append(ChatColor.RED)
					.append(" 被 ")
				...
				Bukkit.broadcastMessage(new StringBuffer()
				.append(ChatColor.BOLD).append(ChatColor.LIGHT_PURPLE)
				.append("末影龙")
				.append(ChatColor.RED)
				.append(" 被 ")
				.append(ChatColor.YELLOW)
				...
				if(DeadEntity.getWorld().getName().equals("world_the_end")){
					if(Math.abs(DeadEntity.getLocation().getBlockX())>150||Math.abs(DeadEntity.getLocation().getBlockY())>150){
		...
		if(DeadEntity.hasMetadata("isSpawner")&&NotInCat){
			...
			if(DeadEntity instanceof Blaze){
				...
				if(SpawnerExpRate<2){
					...
					Bukkit.getScheduler().scheduleSyncDelayedTask(EMM, 
						new Runnable(){
							public void run(){
								Location DeadLocation = DeadEntity.getLocation();
								if(DeadLocation.getBlock().getType().equals(Material.WATER))DeadLocation.getBlock().setType(Material.AIR);
								DeadEntity.getWorld().createExplosion(DeadLocation, 2);
								DeadEntity.getWorld().createExplosion(DeadLocation.getX(), DeadLocation.getY(), DeadLocation.getZ(), 5, false, false);
							}
					}, 3*20L);
				}
	...
	public void ZombieDiamondMiner(LivingEntity Miner){
		...
		((Zombie)Miner).setVillager(false);						//不能产生村民僵尸形态
		...
		Miner.getEquipment().setItemInHand(new ItemStack(Material.DIAMOND_PICKAXE));				//手持钻石镐
		...
		Miner.getEquipment().setItemInHandDropChance(0.5F);		//手中物品掉落概率5%
		Miner.getEquipment().setHelmetDropChance(0.25F);			//钻石块掉落概率20%
		Miner.setMaxHealth(28D);								//28点血
	...
	public void ZombieGiantGhost(LivingEntity GiantGhost){
		...
		GiantGhost.remove();
		//CreatureSpawnEvent e = new CreatureSpawnEvent(GiantZombie,SpawnReason.NATURAL);
		...
		GiantZombie.setMaxHealth(60D);								//血量50
		GiantZombie.setHealth(60D);								//血量50
		GiantZombie.getLocation().getChunk().unload();
	}
	//狂战骷髅
	public void SkeletonCrazySkeleton(LivingEntity CrazySkeleton){
		...
		CrazySkeleton.getEquipment().setItemInHand(new ItemStack(Material.DIAMOND_SWORD, 1));		//钻石剑
		...
		CrazySkeleton.getEquipment().setItemInHandDropChance(0.1F);		//手中掉落10%
		CrazySkeleton.getEquipment().setHelmetDropChance(0.1F);			//头盔掉落10%
		CrazySkeleton.setMaxHealth(35D);							//血量35
	...
	public void SkeletonDarkAzrael(LivingEntity DarkAzrael){
		...
		DarkAzrael.getEquipment().setHelmet(new ItemStack(Material.SKULL_ITEM));								//头戴骷髅头
		...
		DarkAzrael.getEquipment().setItemInHand(StoneHoe);				//手持死神镰刀
		DarkAzrael.getEquipment().setItemInHandDropChance(0.1F);			//死神镰刀掉落概率10%
		...
		DarkAzrael.setMaxHealth(50D);								//血量50
	...
	public void SkeletonDemonSkeleton(LivingEntity DemonSkeleton){
		...
		DemonSkeleton.getEquipment().setHelmet(new ItemStack(Material.MOB_SPAWNER));		//头戴刷怪笼
		DemonSkeleton.getEquipment().setItemInHand(new ItemStack(Material.IRON_PICKAXE));	//手持铁镐
		...
		DemonSkeleton.getEquipment().setItemInHandDropChance(0.1F);		//铁镐掉落率10%
		...
		DemonSkeleton.setMaxHealth(40D);							//血量40
	...
	public void SlimeCrazySlime(LivingEntity CrazySlime){
		...
		CrazySlime.setMaxHealth(70D);								//血量60
	...
	public void onEntitySpawn(CreatureSpawnEvent event){
		...
		if(event.getEntityType().equals(EntityType.IRON_GOLEM)){
			if(event.getSpawnReason()==SpawnReason.VILLAGE_DEFENSE||event.getSpawnReason()==SpawnReason.VILLAGE_INVASION){
		...
		if(event.getSpawnReason()==SpawnReason.SPAWNER&&NotInCat){
			...
			Spawned.getEquipment().setItemInHandDropChance(0.001F);
			int NearCount = 0;
			switch(entityType){
			case ZOMBIE:
			...
			switch (entityType)
			{
				...
				case PIG_ZOMBIE:
					if(Spawned.getWorld().getName().equals("world_nether")){
						...
						Spawned.setMaxHealth(35D);
						Spawned.setHealth(35D);
					}
					break;
				case ENDERMAN:
					if(Spawned.getWorld().getName().equals("world_the_end")){
						if(Math.abs(Spawned.getLocation().getBlockX())>150||Math.abs(Spawned.getLocation().getBlockY())>150){
							int Endermanrate = 100;
							random = 1 + (int)(Math.random() * (double)((Endermanrate - 1) + 1));
							if(random>=8){
								Spawned.remove();
							}else{
								((Enderman)Spawned).setMaxHealth(70D);
								((Enderman)Spawned).setHealth(70D);
							}
						}
					}
					break;
